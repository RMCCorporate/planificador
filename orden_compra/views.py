...
def fecha_respuesta_editar_precio(cotizaciones):
    ...
    for i in cotizaciones:
        if i.orden_compra is not True and i.fecha_respuesta:
            diccionario = {}
...
def crear_orden(request, id):
    if request.method == "GET":
        ...
        productos_proyecto = [Producto_proyecto.objects.get(producto=cotizacion_padre.proyecto_asociado, proyecto=producto)
                              for producto in cotizacion_padre.productos_asociados.all()]
        payload = {"cotizacion_padre": cotizacion_padre, "productos": productos_proyecto}
        return render(request, "orden_compra/crear_orden.html", payload)
    else:
        post = request.POST
        ...
        cotizacion_padre = Cotizacion.objects.get(id=post["id_cotizacion"])
        ...
        nueva_cotizacion.save()
        for i in post.getlist("id_producto"):
            ...
            nueva_cotizacion.save()
        if RMC.objects.filter(rut=post["empresa"]).exists():
            destino_factura = RMC.objects.filter(rut=post["empresa"])[0]
        else:
            destino_factura = empresa(post["empresa"])
        nueva_orden_compra = Orden_compra(
            id=id_orden,
            cotizacion_padre=cotizacion_padre,
            cotizacion_hija=nueva_cotizacion,
            condicion_entrega=post["condicion_entrega"],
        ...
        for i in planificadores:
            ...
            usuario_planificador.save()
        usuario_cotizador = get_user_model().objects.get(correo=request.user.correo)
...
def editar_orden(request, id):
    ...
    if request.method == "GET":
        productos = orden_compra.cotizacion_hija.productos_proyecto_asociados.all()
        payload = {"orden_compra": orden_compra, "productos": productos}
        return render(request, "orden_compra/editar_orden.html", payload)
    else:
        post = request.post
        productos = post.getlist("id_producto")
        orden_compra.condicion_entrega = post["condicion_entrega"]
        orden_compra.condiciones_pago = post["condicion_pago"]
        orden_compra.forma_pago = post["forma_pago"]
        orden_compra.destino_factura = RMC.objects.get(rut=post["empresa"])
        orden_compra.observaciones = post["observaciones"]
        fecha_envio = date.today()
...
def editar_status(request, id):
    ...
    if request.method == "GET":
        ...
        post = request.POST
        orden_compra.status_financiero = post["status_financiero"]
        orden_compra.status_llegada = post["status_llegada"]
        orden_compra.save()
        return redirect(
            "/proyectos/mostrar_cotizacion/{}".format(orden_compra.cotizacion_hija.id)
        )
...
def info_gasto(request, id):
    ...
    if request.method == "POST":
        ...
        return render(request, "orden_compra/info_gasto.html", payload)
